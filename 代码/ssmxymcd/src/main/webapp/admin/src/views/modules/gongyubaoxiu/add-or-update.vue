<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="报修标题" prop="baoxiubiaoti">
          <el-input v-model="ruleForm.baoxiubiaoti" 
              placeholder="报修标题" clearable  :readonly="ro.baoxiubiaoti"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="报修标题" prop="baoxiubiaoti">
              <el-input v-model="ruleForm.baoxiubiaoti" 
...
export default {
  ...
  methods: {
	...
	this.ruleForm.baoxiuneirong = this.ruleForm.baoxiuneirong.replace(reg,'../../../ssmxymcd/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `gongyubaoxiu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.gongyubaoxiuCrossAddOrUpdateFlag = false;
                  this.parent.search();
